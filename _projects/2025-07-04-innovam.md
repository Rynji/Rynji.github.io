...
At [Innovam](https://www.innovam.nl/over-innovam), I work as a Unity Developer on advanced 3D, AR & VR applications within the automotive domain; including interactive EV training systems, real-time AR product visualizations, and scalable simulation frameworks. I contribute across architecture, content implementation, and system design. Over the past year, agentic AI coding tools have also become a core part of my workflow here: I direct them to speed up implementation while keeping architecture and code review in my own hands, especially valuable when working within large existing codebases. On this page you can read more about the work I am currently doing at Innovam.
...
The app is built using a modular, step-based architecture, allowing new training modules to be created with minimal overhead by reusing core interaction logic. I implemented full localisation support using Unity's Localization package, making the app accessible to a wider audience.
...
Players can customize their characters with a fully in-house character creator programmed by me and with 3d assets from our own artists. The character creator also comes with a full in-editor (Unity Editor) wizard-like wearable creation tool where artists can go from an fbx import to a fully setup wearable item for in the creator. It handles binding thumbnail images, colour collection, slot information etc. all in an easy to use wizard like format.
* **Bicycle & pedestrians traffic intensity visualization** <br>
I built a 3D data visualization system that shows admin configurable both bicycle & pedestrian flow per spline path, displaying how many cyclists or pedestrians pass through a given area each hour. The visualization is synchronized for all connected players, providing a consistent and shared data experience. This way residents can get a feel for what a certain amount of cyclist and/or pedestrians per hour through their neighbourhood or street looks like. The tool comes with a spline creation wizard with custom editor to setup splines throughout virtual cities projects. Splines can either be dynamic with synchronized population values or static with a single value to dress up project areas within Virtual Cities.
<br><br>
